...
class UnitreeGo2EnvRL(PipelineEnv):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.get_value:
            ...
            value_model_path = os.path.join(RL_LOG_DIR, value_path) 
        ...
        if self.deploy:
            ...
            self.reward_config.rewards.scales.tracking_lin_vel = 0.
            self.reward_config.rewards.scales.tracking_ang_vel = 0.
            self.reward_config.rewards.scales.lin_vel_z = 0.
            self.reward_config.rewards.scales.ang_vel_xy = -0.
            self.reward_config.rewards.scales.action_rate = 0.0
            self.reward_config.rewards.scales.torques = -0.00
            self.reward_config.rewards.scales.termination = -0.
            self.reward_config.rewards.scales.orientation = -0.0
            self.reward_config.rewards.scales.stand_still = 0.
            self.reward_config.rewards.scales.feet_air_time = 0.
            self.reward_config.rewards.scales.foot_slip = -0.005

            self.reward_config.rewards.scales.gaits = 0.1
            self.reward_config.rewards.scales.energy = 0.
            self.reward_config.rewards.scales.height = 0.5 
            self.reward_config.rewards.scales.upright = 0.5 
            self.reward_config.rewards.scales.vel = 1.
            self.reward_config.rewards.scales.ang_vel = 1.
    ...
    def step(
        ...
        if self.deploy:
            rewards.update({
                ...
                'ang_vel': self._reward_ang_vel(pipeline_state, state.info['command']),
    ...
    def _reward_height(self, pipeline_state: base.State) -> jax.Array:
        ...
        reward_height = -jnp.sum(
            (x.pos[self._torso_idx - 1, 2] - 0.3) ** 2
        )
    ...
    def _reward_ang_vel(self, pipeline_state: base.State, commands: jax.Array) -> jax.Array:
        x, xd = pipeline_state.x, pipeline_state.xd
        ab = global_to_body_velocity(
            xd.ang[self._torso_idx - 1] * jnp.pi / 180.0, x.rot[self._torso_idx - 1]
        )
