...
#       return joint_targets
  ...
  def step_rsl(self, state: State, action: jax.Array) -> State:
      ...
      state_ret = jax.lax.cond(state.done, lambda x: state_reset, lambda x: state, state.done)
...
class UnitreeH1EnvRL(PipelineEnv):
  ...
  def __init__(
    ...
    self._torso_idx = mujoco.mj_name2id(sys.mj_model, mujoco.mjtObj.mjOBJ_BODY.value, 'torso_link')
    self._action_scale = action_scale
    self._obs_noise = obs_noise
    self._kick_vel = kick_vel
    self._init_q = jp.array(sys.mj_model.keyframe('home').qpos)
    ...
    # self.lowers = jp.array([-0.8, -1.2, -2.5] * 4)
    # self.uppers = jp.array([0.8, 2.3, -0.6] * 4)
    self.lowers = jp.array([-0.43, -0.43, -1.57, -0.26, -0.87, -0.43, -0.43, -1.57, -0.26, -0.87, -2.35,
          -2.87, -0.34, -1.3, -1.25, -2.87, -3.11, -4.45, -1.25])
    self.uppers = jp.array([0.43, 0.43, 1.57, 2.05, 0.52, 0.43, 0.43, 1.57, 2.05, 0.52, 2.35,
          2.87, 3.11, 4.45, 2.61, 2.87, 0.34, 1.3, 2.61])
    self.num_actions = 19
    feet_site = [
        "left_ankle_link",
        "right_ankle_link",
