...
from sac_mppi.dial_mpc.dial_mpc.utils.function_utils import global_to_body_velocity, get_foot_step
...
log_dir = os.path.join(log_root, datetime.now().strftime('%b%d_%H-%M-%S') + '_' + "walk")
...
train_fn = functools.partial(
    train, num_timesteps=100_000_000, num_evals=10, reward_scaling=1.,
...
def progress(num_steps, metrics):
  ...
  print(f'step: {num_steps}, reward: {metrics["eval/episode_reward"]:.3f}, time: {times[-1] - times[0]}')
...
make_inference_fn, params, value_params, _= train_fn(environment=env, progress_fn=progress)
